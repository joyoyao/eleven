...
public final class MusicUtils {
    ...
    public static final long[] getSongListForCursor(Cursor cursor) {
        ...
        int columnIndex = -1;
        try {
            columnIndex = cursor.getColumnIndexOrThrow(Playlists.Members.AUDIO_ID);
    ...
    public static final long getIdForPlaylist(final Context context, final String name) {
        Cursor cursor = context.getContentResolver().query(
                Playlists.EXTERNAL_CONTENT_URI, new String[]{
                        BaseColumns._ID
    ...
    public static final long createPlaylist(final Context context, final String name) {
        if (name != null && name.length() > 0) {
            ...
            Cursor cursor = resolver.query(Playlists.EXTERNAL_CONTENT_URI,
                    projection, selection, null, null);
            if (cursor.getCount() <= 0) {
                final ContentValues values = new ContentValues(1);
                values.put(PlaylistsColumns.NAME, name);
                final Uri uri = resolver.insert(Playlists.EXTERNAL_CONTENT_URI,
                        values);
